document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NEW FINGERBOARD DATA STRUCTURE ---
    const fingerboardLayout = [
        // This array represents the rows from your image
    ...
    // --- UPDATED NOTE LISTS FOR THE GAME ---
    // Automatically generate the list of all notes from the layout
    ...
    const noteFrequencies = {
        ...
        'A4': 440.00, 'B4': 493.88, 'C5': 523.25, 'C#5': 554.37, 'D5': 587.33, 'E5': 659.25,
        'E5': 659.25, 'F5': 698.46, 'F#5': 739.99, 'G5': 783.99, 'G#5': 830.61, 'A5': 880.00, 'B5': 987.77
    };
    ...
    async function loadAudioFile(note) {
        if (audioBuffers[note]) return; // Don't load twice
        try {
            ...
            // Fallback: if audio file fails, use synthesized tone
            if (!noteFrequencies[note]) console.error(`Frequency for note ${note} not found.`);
        }
    }
    ...
    function createFingerboard() {
        ...
        fingerboardLayout.forEach(row => {
            ...
            // Add the finger label at the end of the row
